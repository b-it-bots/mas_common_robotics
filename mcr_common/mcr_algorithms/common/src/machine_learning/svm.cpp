#include "mcr_algorithms/machine_learning/svm.h"
...
void SVM::train(char* pInputSampleFileName, char* OutputModelFilename, double &dRetTrainError, double &dRetCrossValError)
{
	...
	//check parameters
	svm_check_parameter(&strSvmProblem, &strSvmParameters);
	...
	//clean up
	svm_free_model_content(pstrSvmModel);
...
int SVM::test(char* data_filename, const char* filename_model, double &dTestError)
{
	int correct = 0;
	int total = 0;
	double error = 0;
...
int SVM::crossValidationParameters(char* pSampleFilename, char* pFixedTestFilename, char* pStatisticsFilename)
{
	...
	const char *pTempModelFilename = "tempSvmModel.tmp";
	double dGamma = -1;
	double dC = -1;
	char tempText[1000];
