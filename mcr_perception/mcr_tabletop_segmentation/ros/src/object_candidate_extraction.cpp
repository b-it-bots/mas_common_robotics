...
void CObjectCandidateExtraction::extractObjectCandidates(pcl::PointCloud<pcl::PointXYZRGB> &point_cloud,
			...
			for (unsigned int j = 0; j < total_point_cloud_size; ++j)
			{
				...
				if (toolBox.pointInsideConvexHull2d(cloud_hull, total_point_cloud.points[j])
				        && total_point_cloud.points[j].z
				                > (toolBox.getNearestNeighborPlane(*hierarchyPlanes[indexMaxPointsClusteredPlane], total_point_cloud.points[j]).z
				                        + THRESHOLD_POINT_ABOVE_LOWER_PLANE))
				...
				{
					reject = false;
					for (unsigned int iterUpperPlanes = 0; iterUpperPlanes < hierarchyPlanes[indexMaxPointsClusteredPlane]->upperPlanarSurfaces.size();
					        iterUpperPlanes++)
					{
