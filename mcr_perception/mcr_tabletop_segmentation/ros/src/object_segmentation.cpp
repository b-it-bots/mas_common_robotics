...
ObjectSegmentation::ObjectSegmentation()
{
	...
	_objects_pub = pn.advertise < mcr_perception_msgs::ObjectList > ("segmented_objects", 1);
	ROS_INFO("Publishing on 'segmented_objects' topic");

	_get_segmented_objects_srv = pn.advertiseService("get_segmented_objects", &ObjectSegmentation::GetObjectList, this);
...
bool ObjectSegmentation::GetObjectList(mcr_perception_msgs::GetObjectList::Request &req, mcr_perception_msgs::GetObjectList::Response &res)
{
	...
	try
	{
		...
		std::vector<mcr_perception_msgs::Object> segmented_objects;
		...
		for (unsigned int i = 0; i < hierarchy_planes.size(); i++)
		{
			...
			for (unsigned int j = 0; j < plane->clusteredObjects.size(); j++)
			{
				...
				mcr_perception_msgs::Object segmented_object;
...
sensor_msgs::ImagePtr ObjectSegmentation::ExtractRegionOfInterest(const sensor_msgs::PointCloud2 &cluster, IplImage *image)
{
	...
	if (roi.x_offset >= (unsigned int) image->width)
		roi.x_offset = image->width - 1;
	if (roi.y_offset >= (unsigned int) image->height)
		roi.y_offset = image->height - 1;
	if ((roi.x_offset + roi.width) >= (unsigned int) image->width)
		roi.width = image->width - roi.x_offset - 1;
	if ((roi.y_offset + roi.height) >= (unsigned int) image->height)
		roi.height = image->height - roi.y_offset - 1;
	...
	IplImage *sub_image = cvCreateImage(cvGetSize(image), image->depth, image->nChannels);
	...
	out_msg.image = sub_image;  // Your cv::Mat
