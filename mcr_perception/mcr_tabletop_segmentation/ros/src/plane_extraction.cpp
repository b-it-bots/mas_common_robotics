...
pcl::PointCloud<pcl::PointXYZRGBNormal> CPlaneExtraction::extractHorizontalSurface(pcl::PointCloud<pcl::PointXYZRGBNormal> &point_cloud, bool surface)
{
	//ROS_DEBUG("before in %d", (int)point_cloud.points.size ());

	Eigen::Vector3f v = Eigen::Vector3f(0, 0, 1.0);  //x

	pcl::PointCloud<pcl::PointXYZRGBNormal> cloud_Inliers, cloud_projected, total_point_cloud;
...
pcl::PointCloud<pcl::PointXYZRGB> CPlaneExtraction::extractHorizontalSurfaceFromNormals(pcl::PointCloud<pcl::PointXYZRGB> &point_cloud, bool surface)
{
	...
	seg.setInputCloud(boost::make_shared < pcl::PointCloud<pcl::PointXYZRGB> > (point_cloud));
	...
	proj.setInputCloud(boost::make_shared < pcl::PointCloud<pcl::PointXYZRGB> > (point_cloud));
	proj.setModelCoefficients(boost::make_shared < pcl::ModelCoefficients > (coefficients));
	proj.filter(cloud_projected);
...
std::vector<StructPlanarSurface*> CPlaneExtraction::extractMultiplePlanes(
        ...
        std::vector<pcl::PointCloud<pcl::PointXYZRGBNormal>, Eigen::aligned_allocator<pcl::PointCloud<pcl::PointXYZRGBNormal> > > &clustered_planes, int axis)
{
	...
	for (unsigned int iter = 0; iter < point_cloud_normal.points.size(); iter++)
	{
		...
		if (point_cloud_normal.points[iter].normal[axis] < (NORMAL_THRESHOLD * (-1)) || point_cloud_normal.points[iter].normal[axis] > NORMAL_THRESHOLD)  //0.9
		{
	...
	extractIndices.setIndices(boost::make_shared<const pcl::PointIndices>(planar_inliers));
	...
	{
		pcl::copyPointCloud(planar_point_cloud_normal, clusteredObjectIndices.at(iterCluster), clustered_planes.at(iterCluster));
	}
	//------------------------------------------------------
	...
	finish = ros::Time::now();
	std::vector<StructPlanarSurface*> surfacesPH = this->createPlanarHierarchy(clustered_planes, DO_MULTI_PLANE);
        ...
        bool doMultiplane)
{
	...
	for (unsigned int iterCluster = 0; iterCluster < clustered_planes.size(); iterCluster++)
	{
		...
		convexHullExtractor.setInputCloud(boost::make_shared < pcl::PointCloud<pcl::PointXYZRGBNormal> > (planarSurface->pointCloud));
		...
		planarSurface->tree = boost::make_shared<pcl::KdTreeFLANN<pcl::PointXYZRGBNormal> >();
		planarSurface->tree->setInputCloud(boost::make_shared < pcl::PointCloud<pcl::PointXYZRGBNormal> > (planarSurface->pointCloud));
		...
		for (unsigned int iterCheckJoin = 0; iterCheckJoin < planarSurfaces.size(); iterCheckJoin++)
		{
			if ((fabs(planarSurface->plane_height - planarSurfaces[iterCheckJoin]->plane_height) < THRESHOLD_JOIN_PLANES_HEIGHT)
			        && (toolBox.distanceBetweenPlane2d(*planarSurface, *planarSurfaces[iterCheckJoin],
			        THRESHOLD_JOIN_PLANES_DISTANCE)))
			{

				ROS_DEBUG("[createPlanarHierarchy] Joining Candidate found (%f)!",
				          fabs(planarSurface->plane_height - planarSurfaces[iterCheckJoin]->plane_height));
				planarSurfaces[iterCheckJoin]->pointCloud += planarSurface->pointCloud;
				...
				convexHullExtractor.reconstruct(planarSurfaces[iterCheckJoin]->convexHull);
				//reconstruct does not fill width and height of the pointcloud

				//
				planarSurfaces[iterCheckJoin]->area = toolBox.areaConvexHull2d(planarSurfaces[iterCheckJoin]->convexHull);

				planarSurfaces[iterCheckJoin]->centroid = toolBox.centroidHull2d(planarSurfaces[iterCheckJoin]->convexHull, planarSurface->area);
				planarSurfaces[iterCheckJoin]->plane_height = toolBox.avgValuePointCloud3d(planarSurfaces[iterCheckJoin]->convexHull, 2);
				...
				planarSurfaces[iterCheckJoin]->tree = boost::make_shared<pcl::KdTreeFLANN<pcl::PointXYZRGBNormal> >();
				planarSurfaces[iterCheckJoin]->tree->setInputCloud(
				        boost::make_shared < pcl::PointCloud<pcl::PointXYZRGBNormal> > (planarSurfaces[iterCheckJoin]->pointCloud));
	...
	{
		ROS_DEBUG("[createPlanarHierarchy] %d. planarSurfaces height = %f, area = %f", planarSurfaces.at(iter)->id, planarSurfaces.at(iter)->plane_height,
		          planarSurfaces.at(iter)->area);
	}
	...
	bool overlap = false;
	for (unsigned int iterLowerPlanes = 0; iterLowerPlanes < numberplanarSurfaces; iterLowerPlanes++)
	{
		for (unsigned int iterUpperPlanes = iterLowerPlanes + 1; iterUpperPlanes < numberplanarSurfaces; iterUpperPlanes++)
		{
			...
			if (toolBox.overlapConvexHull2d2(*planarSurfaces[iterLowerPlanes], *planarSurfaces[iterUpperPlanes]))
			{
				overlap = false;
				...
				if (planarSurfaces[iterLowerPlanes]->upperPlanarSurfaces.size() > 0)
				{
					//  start = ros::Time::now();
					for (unsigned int iterOverlap = 0; iterOverlap < planarSurfaces[iterLowerPlanes]->upperPlanarSurfaces.size(); iterOverlap++)
					{
						if (toolBox.overlapConvexHull2d2(*planarSurfaces[iterLowerPlanes]->upperPlanarSurfaces[iterOverlap], *planarSurfaces[iterUpperPlanes]))
						{
							ROS_DEBUG("[createPlanarHierarchy] %d. planarSurfaces overlap %d(%d) lower plane", planarSurfaces[iterUpperPlanes]->id,
							          planarSurfaces[iterLowerPlanes]->upperPlanarSurfaces[iterOverlap]->id, planarSurfaces[iterLowerPlanes]->id);
							overlap = true;
							break;
						}
						// old test
				...
				if (!overlap)
				{  //Add upper to lower plane!
					planarSurfaces[iterLowerPlanes]->upperPlanarSurfaces.push_back(planarSurfaces[iterUpperPlanes]);
					ROS_DEBUG("[createPlanarHierarchy] %d. planarSurfaces is inside in %d lower plane --> %d plane added", planarSurfaces[iterUpperPlanes]->id,
					          planarSurfaces[iterLowerPlanes]->id, planarSurfaces[iterUpperPlanes]->id);
				}
