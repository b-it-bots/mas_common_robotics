...
// -----------------------------------------------------------------------
	...
	void Args::declareArgument(const string& name, const string& description, int numValues, const string& valuesNamesList)
	{
	...
	ArgsOutType Args::readArguments(int argc, char* argv[])
	{
		...
		for (int i = 1; i < argc; ++i)
		{
			...
			if (!argFound)
			{
				...
				for (mm_iterator it = range.first; it != range.second; ++it)
				{
					cerr << " " << _argDiscriminator << argName << " " << it->second->valuesNamesList << endl;
