...
#include "weak_learners/haar_single_stump_learner.h"
...
// ------------------------------------------------------------------------------
	...
	double HaarSingleStumpLearner::classify(InputData* pData, int idx, int classIdx)
	{
		...
		return _v[classIdx] * SingleStumpLearner::phi(_pSelectedFeature->getValue(static_cast<HaarData*>(pData)->getIntImage(idx), _selectedConfig), classIdx);
	...
	void HaarSingleStumpLearner::run(InputData* pData)
	{
		...
		for (ftIt = loadedFeatures.begin(); ftIt != loadedFeatures.end(); ++ftIt)
		{
			...
			while (pCurrFeature->hasConfigs())
			{
				...
				findThreshold<int>(processedHaarData.begin(), processedHaarData.end(), pData, tmpThreshold, mu, tmpV);
...
// -----------------------------------------------------------------------
	...
	void HaarSingleStumpLearner::getStateData(vector<double>& data, const string& /*reason*/, InputData* pData)
	{
		...
		{
			data[pos++] = SingleStumpLearner::phi(_pSelectedFeature->getValue(static_cast<HaarData*>(pData)->getIntImage(i), _selectedConfig), 0);
		}
	}
