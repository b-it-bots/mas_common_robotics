...
#ifndef __SORTED_DATA_CPP
#define __SORTED_DATA_CPP

#include "io/sorted_data.h"

#include "defaults.h" // for STABLE_SORT declaration
#include "utils/utils.cpp" // for comparePairOnSecond
#include <algorithm> // for sort
#include "general.h" // for typedefs @ggiorgana
// ------------------------------------------------------------------------
...
// Test does not need sorting
...
#else
			sort(_sortedData[j].begin(), _sortedData[j].end(), nor_utils::comparePairOnSecond<int, double, less<double> >);
#endif
		}
	...
	void SortedData::loadTrain(vector3Df &trainData, vector1Di &labels)
	{
		eInputType inputType = IT_TRAIN;
		int verboseLevel = 1;

		//InputData::loadTrain(trainData, labels);
		...
		// The data is stored column-wise. The index [j] is the column
		// and the pair represent the index of the example with the value
		std::cout << std::endl << std::endl << "nExamples: " << _numExamples << std::endl;
		for (int i = 0; i < _numExamples; ++i)
		{
			//std::cout<<std::endl <<std::endl <<"example " <<i <<": " <<"column: " <<std::endl;
			for (int j = 0; j < _numColumns; ++j)
			{
				//std::cout <<j <<" ";
				_sortedData[j].push_back(make_pair(i, _data[i][j]));
		...
		//////////////////////////////////////////////////////////////////////////
		// Now sort the data.
		std::cout << std::endl << std::endl << "Now sort the data...";
