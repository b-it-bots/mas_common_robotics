...
#ifndef __HAAR_DATA_CPP
#define __HAAR_DATA_CPP
...
// ------------------------------------------------------------------------
	...
	void HaarData::load(const string& fileName, eInputType inputType, int verboseLevel)
	{
		...
		if (_width == 0 || _height == 0)
		{
			...
			{
				cerr << "Error: the number of elements in file <" << fileName << "> for each example" << endl << "is not square." << endl
				                                                              << "Please make sure you are loading integral image data, or specify the size"
				                                                              << endl << "of it with -iisize." << endl;
				exit(1);
			}
		...
		if (verboseLevel > 1)
		{
			...
			for (int l = 0; l < numClasses; ++l)
				cout << "Of class '" << ClassMappings::getClassNameFromIdx(l) << "': " << _nExamplesPerClass[l] << endl;
