...
#include "others/haar_features.cpp"
...
// A couple of useful typedefs
	...
	class HaarData : public InputData
	{
 	public:
		...
		{
			assert(!"This function should not be called with HaarData!!");
			return 0;
		}
		 ...
		 * @date 17/12/2005
		 */
		const int* getIntImage(int idx) const
		{
			return _intImages[idx];
		}
		 ...
		 * @date 17/12/2005
		 */
		const vector<int*>& getIntImageVector() const
		{
			return _intImages;
		}
		 ...
		 * @date 17/12/2005
		 */
		vector<HaarFeature*>& getLoadedFeatures()
		{
