...
//////////////////////////////////////////////////////////////////////////
	...
	class HaarFeature
	{
 	public:
		 ...
		 * @date 27/12/2005
		 */
		HaarFeature(short width = 0, short height = 0, const string& shortName = "", const string& name = "", eFeatureType type = FEATURE_NO_TYPE);
		 ...
		 * @date 27/12/2005
		 */
		void setAccessType(eAccessType accessType)
		{
		 ...
		 * @date 27/12/2005
		 */
		const nor_utils::Rect& getCurrentConfig() const
		{
			return *_configIt;
		}
		 ...
		 * @date 27/12/2005
		 * @todo Replace the "visited" list with something less awful.
		 */
		 ...
		 * @date 27/12/2005
		 */
		void fillHaarData(const vector<int*>& intImages,  // in
		 ...
		 * @date 27/12/2005
		 */
		virtual int getValue(const int* pIntImage, const nor_utils::Rect& r) = 0;
 	...
 	protected:
		 ...
		 * @date 27/12/2005
		 */
		int getSumAt(const int* pIntImage, int x, int y);
 	...
 	private:
		        ...
		        vector<pair<int, int> >& haarData)  // output
		{
			...
			for (iiIt = intImages.begin(), hIt = haarData.begin(), i = 0; iiIt != iiEnd; ++iiIt, ++hIt, ++i)
			{
				hIt->first = i;
